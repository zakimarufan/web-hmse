...
$(document).ready(function() {
    ...
    navigator.userAgent.indexOf('Safari') !== -1 && !isMobileView &&
    navigator.userAgent.indexOf('Chrome') === -1) {
    document.body.classList.add('safari-mac');
...
function hasScroll(el) {
    ...
    return ((el.scrollHeight > el.clientHeight || el.scrollWidth > el.clientWidth) && (style.overflow == "scroll" || style.overflow == "auto"));
}
...
let child = document.querySelector('.simplebar-content-wrapper');

$(document).mouseover(function(e){
   let celem = $(e.target).attr('id');
   if ((celem != document.querySelector('html')) && (celem != document.querySelector('body')) && (celem !== undefined)) {
        if (hasScroll(document.getElementById(celem))) {
           child = document.getElementById(celem);
        } else {
           child = document.querySelector('.simplebar-content-wrapper');
        }
   } else {
       child = document.querySelector('.simplebar-content-wrapper');
   }
...
//Holding Command key for dedicated purpose while scrolling
...
window.addEventListener('scroll', (event) => {
       if (!disableScrolling && !commandPressed) {
        child.scrollBy({ top: (event.deltaY) / 1.1, behavior: "smooth" });
        }
    window.scrollTo({ top: document.body.scrollHeight, behavior: "instant"});
}, { passive: false });
